...
export function ChatDrawer({
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 z-40 w-full sm:w-96 bg-[#202124] border-l border-[#3c4043] flex flex-col shadow-2xl transition-all animate-in slide-in-from-right duration-200">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#3c4043]">
        ...
        <button
          onClick={onClose}
          className="p-2 rounded-full text-[#9aa0a6] hover:text-white hover:bg-[#303134] transition-colors"
      ...
      {/* Notice info */}
      <div className="px-5 py-3 bg-[#282a2d] border-b border-[#3c4043]/60 text-xs text-[#9aa0a6]">
      ...
      {/* Quick Emojis bar */}
      {showEmojiPicker && (
        <div className="px-4 py-2 bg-[#282a2d] border-t border-[#3c4043] flex items-center gap-2 flex-wrap">
          {QUICK_EMOJIS.map((emoji) => (
            <button
              key={emoji}
              onClick={() => handleEmojiClick(emoji)}
              className="text-xl p-1.5 hover:bg-[#3c4043] rounded-lg transition-transform hover:scale-125"
            >
              {emoji}
            </button>
          ))}
      ...
      {/* Input Box */}
      <form onSubmit={handleSend} className="p-4 border-t border-[#3c4043] bg-[#202124]">
        <div className="flex items-center gap-2 bg-[#303134] rounded-full px-4 py-1.5 focus-within:ring-2 focus-within:ring-[#8ab4f8]">
          <button
            type="button"
          ...
          <input
            ...
            className="flex-1 bg-transparent text-white text-sm outline-none placeholder-[#9aa0a6]"
          />
          <button
            type="submit"
            disabled={!inputText.trim()}
            className="p-1.5 rounded-full bg-[#8ab4f8] text-[#202124] disabled:opacity-40 disabled:hover:bg-[#8ab4f8] hover:bg-[#aecbfa] transition-colors"
